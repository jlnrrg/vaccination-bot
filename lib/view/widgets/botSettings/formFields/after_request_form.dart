...
class AfterRequestForm extends HookWidget {
  const AfterRequestForm({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final value = useProvider(
        botSettingsProvider.select((value) => value.settings.afterRequest));
    return TextFormField(
      ...
      decoration: InputDecoration(
        isDense: true,
        filled: false,
        labelText: LocaleKeys.botSettingsLabel_afterRequest.tr(),
        suffixText: LocaleKeys.units_seconds.tr(),
      ),
      onSaved: (String? value) => context
          .read(botSettingsProvider.notifier)
          .afterRequestChanged(value != null ? int.tryParse(value) : null),
    );
  }
}
